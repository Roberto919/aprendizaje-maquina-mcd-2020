...
<!-- - La función de pérdida es complicada. -->
<!-- - Para no resolver directamente, usamos el gradiente de la pérdida. -->
...
Fijaremos el número de árboles en 100, de profundidad 4, y estimamos el error con
...
Mostraremos ahora cómo controlar
...
Veamos que efecto tiene en nuestro ejemplo. Reducimos considerablemente
...
- Valores bajos de $\eta$ requieren más iteraciones para reducir el error, pero
pueden reducir el sobreajuste.
...
las variables de entrada. Podemos por ejemplo comenzar probando al azar de la siguiente
rejilla:
...
```{r}
eval_tbl %>% select(-objective, -iter, -tipo, -metrica)
```
...
```{r}
#modelo <- xgb.train(d_entrena, verbose = 1, nrounds = 30000, params = params)
