...
<!-- - La idea general es que no necesitas todos los datos para caluclar el gradiente. Puedes usar solo una parte de los datos para lograrlo. -->
...
<!-- - Con descenso estocástico haces más interaciones chicas que te acercan al resultado. -->
<!-- - Hay muchas formas de detener el descenso estocástico: -->
<!--   - Ir reduciendo gradualmente la tasa de aprendizaje. -->
...
por iteración).
<!-- - Si haces lotes muy chicos te la pasas acomodando datos y jalándolos de memoria. El overhead del cálculo es muy grande. -->
  <!-- - Van tamaños de 10 a 5,000 (este último para millones de datos.) -->
<!-- - Si tomas lotes muy grandes, estás haciendo demasiado trabajo del cálculo. -->
...
<!-- - Como cada iteración se hace con el mismo tamaño de lote, por lo que cada iteración tomará el mismo tiempo. -->
...
<!-- - La idea es que podemos promediar las interacciones ateriores para ajustar la nueva. -->
<!-- - En términos físicos sería como una pelotita que va cayendo y lleva velocidad. La nueva iteración sería como un nuevo golpecito.  -->
<!-- - Esto nos dará como resultado un camino menos caótico, la trayectoria se vuelve más limpia. -->
...
<!-- - Significa unidad con rectificación lineal. -->
<!-- - La idea de la relu es que se desactiva en valores negativos. -->
<!-- - La ventaja es que la derivada es muy fácil de calcular. -->
<!--   - Recordemos que necesitamos la derivada para el backpropagation. -->
<!-- - Un problema de la sigmoidal es que la dervada en los extremos es muy cercana a cero. -->
  <!-- - Con la "relu" no tienes problema de las derivadas similares a cero en los extremos. -->
  <!-- - El problema de los ceros extremos se le llama saturación de unidades. -->
...
<!-- - En rojo vemos la sigmoidal (que habíamos estado viendo). -->
<!-- - La relu es la que está en negro. -->
...
<!-- - Lo que hace es que restringe en cada iteración el modelo al azar. -->
<!-- - Se pueden eliminar más de una neurona a la vez. -->
<!-- - Se quiere hacer un modelo simple, por lo que quitas ciertas unidades al azar. Se reduce varianza o sobreajuste. -->
<!-- - Cuando usas dropout vemos en el ejemplo que la red aprende mejor los patrones importantes de lo que estmos buscando. -->
  <!-- - Esto debido a que no estamos dejando que se realice sobreajuste. -->
<!-- - Con el dropout obligamos a que las unidades trabajen por separado; que calculen algo importante por su cuenta. Porque las fuimos apagando en el entrenamiento. -->
<!-- - Cada interación tiene una arquitectura distinta. El resultado es el promedio de los distintos modelos. -->
...
g_1_pos <- graf_pesos(pesos[, pesos_capa_f > 0], mostrar_facets = TRUE)
g_2_pos <- graf_pesos(pesos[, pesos_capa_f < 0], mostrar_facets = TRUE)
g_1_pos + g_2_pos
```
