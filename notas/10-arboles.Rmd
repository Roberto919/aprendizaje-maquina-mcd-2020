...
<!-- - Redes neuronales tienen su nicho: imágenes, texto, audio. -->
<!--   - Necesitas muchos datos. -->
<!-- - Árboles funciona bien para problemas más generales donde tienes menos datos. -->
<!-- - Los árboles es la misma idea de encontrar distintas interacciones entre variables de entrada. -->
<!--   - CHAID -> Automatic Interacion Detector. -->
...
<!-- - Aquí vemos distintas formas de definir árboles de decisiones para llegar a un resultado. -->
  <!-- - Vemos que hay atributos que se vuelven relevantes en ciertos contextos. -->
<!-- - Puedes capturar relaciones no lineales cortando en distintas ramas. -->
...
<!-- - Aquí vemos como se dividen distintas regiones para hacer predicciones. -->
...
más puros (que una sola clase domine).
  <!-- - Esto es un problema de clasificación. -->
  <!-- - Buscas eliminar las impurezas. -->
2. ¿Cuándo declarar a un nodo como terminal? ¿Cuándo particionar más profundamente? Idea: dependiendo de la aplicación, buscamos hacer árboles
  <!-- - Crear un árbol grande y después pódalo. -->
...
común en cada nodo terminal (la de máxima probabilidad).
  <!-- - Ver cuántas clases hay en cada nodo y después eliges. -->
...
<!-- - Queremos que los nodos tengan solo de un clase. -->
...
```
<!-- - El método de CART implica hacer un árbol enorme, intentar que los nodos queden puros, después vas podando. -->
...
```{r}
arbol_grande_1 <- pull_workflow_fit(arbol_grande)
...
el bagging manualmente y calculamos sensibilidad y especificidad:

```{r, message=FALSE}
preds_clase <- map(arboles_bagged, function(arbol){
  preds <- predict(arbol, spam_prueba, type = "prob")[,2]
})
preds <- preds_clase %>% bind_cols()
dim(preds)
prob_bagging <- apply(preds, 1, mean)
# matriz de confusión
table(prob_bagging > 0.5, spam_prueba$spam) %>% 
...
```{r}
bosque <- pull_workflow_fit(flujo_ajustado)
```
...
1. Una desventaja de esta técnica es que tenemos que separar conjuntos de calibración
y evaluación.
2. Para el caso multiclase, se puede probar el siguiente procedimiento: calibrar
...
<!-- - Este es un enfoque más útil porque... (importancia basada en permutaciones). -->
<!-- - En el método de árboles podemos ignorar la información que tenemos de una variable y poner a prueba su "importancia". Dos caminos que podríamos seguir: -->
<!--     - Permutar al azar la columna de la variable. -->
<!--     - Hacer la división del árbol de manerea aleatoria y ver qué pasa con la clasificación. -->
<!-- - Debemos usar los datos oob (out of bag) que son los que no utilizó el árbol. -->
...
```{r}
bosque_casas <- ranger::ranger(SalePrice ~ Neighborhood + MSSubClass +
            LotArea + OverallQual + OverallCond + YearBuilt + GrLivArea +
              GarageArea + Street + CentralAir +
            BsmtQual + TotalBsmtSF + GarageQual + GarageCars + KitchenQual + 
            Functional, data = casas_entrena, 
            mtry = 5, num.trees = 3000, importance = "permutation")
importancias_casas_tbl <- ranger::importance(bosque_casas) %>% enframe() %>% 
