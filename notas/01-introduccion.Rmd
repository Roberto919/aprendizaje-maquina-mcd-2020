...
```
## Hay muchas funciones distintas que se podrían definir, cada una tendría características diferentes
```
...
```
## Se está usando 'sombrerito' porque es un error muestral
```
...
```
## Aquí se está haciendo referencia a la ley de los grandes números. Como no podemos tomar medidas de todo el sistema, podemos aproximar con una muestra.
```
...
<!-- - El error de entrenamiento nunca evalúa el desempeño de in modelo. Lo que debemos tomar en cuenta es el error de prueba. -->
...
grande entre entrenamiento y prueba.
  <!-- - El problema es que está aprendiendo el ruido de los datos. -->
  <!-- - El problema es que hay una brecha muy grande entre entre entrenamiento y prueba. -->
...
la curva 3 en términos de flexibilidad.
  <!-- - Buscamos un punto virtuoso entre flexibilidad y rigidez. -->
...
depende de qué tan bien estimemos $f(X)$ con $\hat{f}(x)$
    ...
    <!-- - El error puede estar dado por el error aleatorio o porque mi función esté muy alejada de la función real. -->
    <!-- - El error reduclible se puede dar cuando: -->
    <!--   1. Tu "f gorro" no puede capturar patrones en los datos que están ahí. (ejemplo, los datos son curvos y tu haces recta) -->
    <!--   2. Tu "f gorro" se va a lo loco con los puntos y captura el ruido. Si tu estimador varía mucho es probable que te de un buen resultado en algún momento. -->
...
- El primer término le llamamos *sesgo* del predictor (si está consistentemente lejos de $f(x)$)
  <!-- - La recta que evaluamos no tiene varianza, por más que corras con nuevos datos siempre saldrá igual. -->
  <!-- - El problema que tiene es el sesgo -->
- El segundo término es la *varianza* del predictor (qué tan sensible es a la muestra de entrenamiento)
  <!-- - Esta se mide con respecto a distintas muestras de entrenamiento. -->
  <!-- - La varianza la evalúas de distinas formas. Hay analíticos, numéricos, etc. -->
  <!-- - La varianza es una ecuación teórica. -->
  <!-- - Esto nos hace depender demasiado de los datos de entrenamiento -->
  <!-- - A esta varianza caótica es a la que le llamamos "sobreajuste" -->
  <!-- - Hay que tener cuidado de no irnos con la finta de que modelos sencillos tienen baja varianza. -->
- Al tercer término le llamamos *error irreducible*.
  <!-- - Expandiendo los valores de "x" puedes obtener menos ruido o entenderlo mejor. -->
<!-- - El objetivo que buscaremos es equilibrar entre el sesgo y la varianza. -->
<!--   - Necesitamos un modelo flexible y rígido a la vez. -->
<!--   - Todos los problemas de optimización y aprendizaje de máquina son problemas de ricitos de oro. -->
<!-- - Normalmente mejorar el sesgo afecta la varianza y viceversa. -->

<!-- - Resumen de preceptos clave -->
<!--   - Los resultados de entrenamiento no te dicen si un modelo es bueno o no. -->
<!--   - Debes usar datos de prueba para comparar. -->
...
- Consideramos una familia de funciones $h$ candidatas para aproximar $f$
  <!-- - Por ejemplo, usar todas las rectas posibles. -->
...
conjuntos de reglas).
  <!-- - Hay que tener cuidado de no sobremuestrar antes de hacer modelos. -->
