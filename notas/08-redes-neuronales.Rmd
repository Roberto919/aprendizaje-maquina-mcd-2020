...
```
<!-- - Con esto vemos que las redes neuronales te permiten hacer combinaciones fáciles entre muchas variables. -->
<!-- - La idea es hacer transformaciones no lineales en las redes neuronales. -->
<!-- - Para el último nodo podríamos aplicar función lineal (para regresión) o no lineal (si queremos probabilidad). -->
...
entradas derivadas.
<!-- -Si la "h" fuera lineal no estaríamos haciendo realmente nada. Lo crucial es que sea no lineal. Se les llama funciones de activación. -->
...
<!-- - Con suficientes datos puedes aproximar la "p" al nivel que tu quieras. -->
...
```
<!-- Podemos ver que podemos simular el chipote tomando en cuenta la diferencia entre las dos curvas. La diferencia es grande en "x" cercanas a 0 y chica en los extremos. -->
...
<!-- Esto se hizo manualmente, pero el chiste es que se haga de forma automatizada. -->
...
<!-- - Los interceptos del código de arriba son los sesgos. -->
<!-- Se le llama "feed_forward" porque meter betas y te da "p" -->
...
```
<!-- - Usando "feed-forward" podemos ir calculando la devianza. -->
...
beta %>% round(2)
```
<!-- - Aquí vemos que los coeficientes están demasiado grandes y pueden requerir regularización. -->
...
ejemplo_mod %>%
  ...
  layer_dense(units = 1,
    activation = "sigmoid", kernel_regularizer = regularizer_l2(0.0005))
```
<!-- - Aquí se están especificando las capas de la red neuronal. -->
...
```
<!-- - El optimizador "sgd" es descenso en gradiente estocástico. -->
<!-- - El batch size se refiere al número de datos que hay que tomar para calcular el gradiente. -->
