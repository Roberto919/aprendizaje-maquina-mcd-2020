...
<!-- - Lo que podríamos hacer medir distintas variables y aplicar un sistema de puntos (lo que hicimos en Rebel) -->
...
<!-- - En este segundo caso agregamos interacciones entre variables. -->
...
<!-- - Solo usamos los de entrenamiento porque no queremos guiar el ajuste. -->
...
<!-- - Aquí podremos ver algunos datos problemáticos como "Partial" que nos meterán ruido al problema. -->
...
graf_ent
```
<!-- - Aquí estamos graficando las dos variables que nos interesan: -->
<!--   - Calidad. -->
<!--   - Si tiene piso adicional o no. -->
...
usamos fue *lm*)? ¿Qué algoritmo se usó para minimizar la suma de cuadrados?
  <!-- - Lo que hicimos fue minimizar el problema de mínimos cuadrados.s -->
...
probados. Estas técnicas se escalan en parte a problemas grandes, pero no es muy simple hacerlo.
  <!-- - Para problemas en los que los datos quepan comodamente en tu memoria funciona bien. -->
...
para escalar a datos grandes y modelos más complejos (por ejemplo redes neuronales).
  <!-- - Este lo vamos a usar normalmente. -->
...
<!-- - Usando mínimos cuadrados normalmente no tendrás el problema de muchos mínimos falsos. -->
<!-- - El descenso por gradiente (y sus métodos estocásticos) puede ser un método primitivo pero la ventaja es que es muy fácil, sencillo y escalable. -->
...
z
```
<!-- - La eta (tasa de aprendizaje) es la más importante. -->
<!-- - La z_0 normalmente la empiezas en 0 y de ahí te mueves. -->
<!--   - En otros problemas más complejos la seleccionas de manera aleatoria. -->
  <!-- - La ventaja de usar "keras" es que se hace diferenciación automática. -->
...
<!-- - El punto de normalizar es hacer que las curvas de nivel sean parecidas, para que no tengamos saltos muy fuertes en una dirección y otros muy débiles. -->
<!-- - Estandarizar no cambia el modelo, solo cambia los escalares. -->
<!-- - La convergencia puede llegar a ser mucho más rápida estandarizando. -->
...
<!-- - En cierto sentido los modelos lineales también son cajas negras. -->
...
2. Entender aspectos del fenómeno de interés a través de nuestro modelo. Por ejemplo,
interpretar un coeficiente positivo como el efecto en nuevos datos de cambiar
una variable que podemos manipular.

<!-- - Hay patrones en los datos que son complicados y, por ende, hacen complicado de interpretar el modelo. -->
<!-- - Generalmente ajustamos datos con muestras observaciones. Hay relaciones complejas entre las variables. -->
<!-- - Los modelos de predicción son para predecir, no para explicar el fenómeno. -->
...
Notamos otro ejemplo en nuestro análisis anterior. Si consideramos
el modelo simple para precio en miles de dólares:

```{r}
modelo_casas_1 <- lm( precio_miles ~ 1 + habitaciones, data = train_tbl)
(coef(modelo_casas_1) %>% round(2))
```

¿Esto implica entonces que mejorar el valor de una casa conviene dividir habitaciones?
Probablemente no. Por ejemplo, para un tamaño de casa fijo, el coeficiente de
número de habitaciones es negativo.

```{r}
modelo_casas_2 <- lm( precio_miles ~ 1 + calidad + habitaciones + habitable_100_m2_c , data = train_tbl)
(coef(modelo_casas_2) %>% round(2))
```

---

Para construir modelos apropiados e interpretaciones adecuadas se requiere tener
las variables y datos importantes, los modelos correctos tanto en términos estadísticos como
en términos de conocimiento de dominio, y entender cómo es el proceso generador de
los datos que estamos utilizando. En general, la recomendación es que las
interpretaciones causales deben considerarse
como preliminares bajo ciertos supuestos, y se requiere más análisis y consideraciones
antes de poder tener interpretaciones causales sólidas (ver @KingCausal y @GelmanHill,
por ejemplo).

## Predicción e incertidumbre

<!-- - Tener intervalos de predicción mejora mucho el proceso de toma de decisiones. -->
<!-- - Tenemos que hacer más trabajo para construir los intervalos predictivos. -->
...
2 a 20 mil), mientras que para cliente B tenemos incertidumbre relativamente baja
(por ejemplo, de 8 mil a 12 mil pesos). Si el tratamiento es costoso, puedes ser que
preferamos tratar a B en lugar de A.
